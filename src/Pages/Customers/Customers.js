...
function Customers() {
  ...
  const handleEdit = (record) => {
    ...
    setFormData({
      ...
      attachments: [], // We don't load existing attachments into the upload array
    });
  ...
  return (
    <div className="container mt-5">
      <Mytitle title={t("customers_title")} />
            ...
            {editId && !canEditOpeningBalance && (
              <div className="text-muted mt-1" style={{ fontSize: "0.9rem" }}>
                {t("cannot_edit_opening_balance_has_transactions") ||
                  "لا يمكن تعديل الرصيد الافتتاحي لوجود معاملات"}
              </div>
            )}
          </div>
        </div>
      ...
      <MyTable
        ...
        button={{
          text: t("customers_addCustomer"),
          onClick: handleAdd,
          variant: "success",
          permission: "employee_create",
        }}
        baseFilter={(item) => item.type === "customer"}
      />
      <div
        style={{
          display: "flex",
