...
- Name: flows.spec.js >> Dashboard Flow >> Dashboard renders company info and charts from mock API
- Location: tests\flows.spec.js:11:3
...
```ts
  ...
  2   | 
  3   | const API_PATTERN = "**/192.168.1.5:8080/**";
  4   | 
  5   | // ──────────────────────────────────────────────
  6   | // Dashboard flow with mocked API
  7   | // ──────────────────────────────────────────────
  8   | test.describe("Dashboard Flow", () => {
  9   |   test.use({ storageState: "playwright/.auth/user.json" });
  10  | 
  11  |   test("Dashboard renders company info and charts from mock API", async ({
  12  |     page,
  13  |   }) => {
  14  |     // Mock the analysis endpoint
  15  |     await page.route(API_PATTERN, async (route) => {
  16  |       const url = route.request().url();
  17  |       if (url.includes("analysis")) {
  ...
  23  |             data: {
  24  |               basic_info: {
  25  |                 company_name: "Ahmed Test Co",
  26  |                 customers_count: 9,
  27  |                 employees_count: 2,
  28  |               },
  29  |               cash_bank_balance_exsist: {
  30  |                 cash_balance: 10594163.56,
  31  |                 bank_balance: 100,
  32  |                 total_balance: 10594263.56,
  33  |               },
  34  |               customer_balance: {
  35  |                 debtors: [
  36  |                   {
  37  |                     customer_id: 15,
  38  |                     customer_name: "Test Customer",
  39  |                     remaining_balance: "8768.00",
  40  |                   },
  41  |                 ],
  42  |                 creditors: [
  43  |                   {
  44  |                     expense_id: 1,
  45  |                     expense_name: "كهرباء",
  46  |                     total_payable: "300.00",
  47  |                   },
  48  |                 ],
  49  |               },
  50  |               totals: {
  51  |                 total_debtors: 8768,
  52  |                 total_creditors: 300,
  53  |               },
  54  |               monthly_revenue_cost: [
  55  |                 {
  56  |                   month: "أبريل",
  57  |                   year: 2026,
  58  |                   total_revenue: 10000,
  59  |                   total_cost: 3000,
  60  |                 },
  61  |               ],
  62  |               top_selling_services: [
  63  |                 { service_name: "بيع سيارة", sales_count: 5 },
  64  |               ],
  65  |             },
  66  |           }),
  67  |         });
  ...
  72  |           body: JSON.stringify({ status: true, data: [], result: [], meta: { total: 0 } }),
  73  |         });
  74  |       }
  75  |     });
  76  | 
